...
// Cells all the way down on this model, so a die is exactly three text rows.
void drawDieSpace(unsigned char x, unsigned char y)
{
    hires_Mask(x, y, 3, 3, BG_FILL);
}
...
void drawDie(unsigned char x, unsigned char y, unsigned char s, bool isSelected, bool isHighlighted)
{
    ...
    src = diceChars + (s - 1) * 9;
    // Only the six faces have a selected variant - the button faces sit past
    // it, so offsetting them would read off the end of the table.
    if (isSelected && s <= 6) src += 54;
    // Highlighted dice flash red; the highlighted buttons use cyan.
...
//
// Must track DICE_Y in gamelogic.c: the die occupies cell rows DICE_ROW..+2 and
// the frame the row above and below.
#define DICE_ROW 18
#define DICE_CURSOR_TOP (DICE_ROW - 1)
...
// (x..x+2, DICE_ROW..DICE_ROW+2), with a 1px gap to the die and 1px-rounded
// corners (the outer row/col is inset one pixel at each end). All touched
// pixels lie within the 16 perimeter cells saved by drawDiceCursor.
static void draw_cursor_frame(unsigned char x)
{
    uint16_t L = (uint16_t)x * 8 - 3, R = (uint16_t)x * 8 + 26;
    uint16_t T = DICE_ROW * 8 - 3, B = DICE_ROW * 8 + 26;
    uint16_t p;
...
uint8_t diceChars[] = {
  ...
  // Bank button. Same rounded corners as a die, no counter on the bottom row.
  0x01, 0x00, 0x02, 0x5E, 0x5F, 0x60, 0x03, 0x00, 0x04, // 17 - Bank

  // Roll with no counter - 14-16 carry a rolls-remaining mark we do not want
  0x01, 0x00, 0x02, 0x0E, 0x0F, 0x10, 0x03, 0x00, 0x04  // 18 - Roll
};
...
uint8_t highlight[] = {0b100000, 0b1010000};
...
void drawTextAlt(unsigned char x, unsigned char y, char* s) {
  static unsigned char c, mustAlt;
    static uint8_t rop;
  // drawLogo() renders the logo at y == 2, so skip the in-game forced
  // alt-color path there and preserve the split "fujiRKLE" styling.
  if (state.inGame && y == 2 && x == WIDTH/2 - 4) {
    mustAlt = false;
  } else {
...
static unsigned char diePixelY(unsigned char y) {
  ...
  // The bottom dice row is nudged down to make room
  if (y == 160)
    y = 165;
...
void drawDie(unsigned char x, unsigned char y, unsigned char s, bool isKept, bool isHighlighted) {
  ...
  // Is die being kept? Only the six faces have a kept variant - the button
  // faces sit past it, so offsetting them would read off the end of the table.
  if (isKept && s<=6)
    source+=54;
...
void drawBoard() {
  ...
  // Game name glyphs - left over scorecard art
  drawGameName(SCORES_X,255);
}

void drawGameName(unsigned char x, unsigned char y) {
  char gameName[] = {0x1b, 0x1c, 0x1d, 0x1e, 0x1e, 0};
  drawText(x,y,&gameName);
}
...
void drawDiceCursor(unsigned char x) {
  ...
  // Top / Bottom
  hires_Mask(x-1,DICE_CURSOR_BASE-28,5,2, ROP_BLUE);
  hires_Mask(x-1,DICE_CURSOR_BASE,5,2, ROP_BLUE);

  // Corners
  hires_Mask(x-1,DICE_CURSOR_BASE-28,1,2, 0x0f & ROP_BLUE);
  hires_Mask(x-1,DICE_CURSOR_BASE,1,2, 0x0f & ROP_BLUE);
  hires_Mask(x+3,DICE_CURSOR_BASE-28,1,2, 0xf0 & ROP_BLUE);
  hires_Mask(x+3,DICE_CURSOR_BASE,1,2, 0xf0 & ROP_BLUE);
}
