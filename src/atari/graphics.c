...
const unsigned char diceChars[] = {
  ...
  // fujirkle's two buttons. Its DIE_ROLL/DIE_BANK are 18 and 17, which collide
  // with fujitzee's pushed-roll block, so drawDie maps them by name instead of
  // by arithmetic. Selected is the same tile inverted, +0x80 per byte.
  0x41, 0x40, 0x42, 0x2C, 0x2D, 0x2E, 0x43, 0x40, 0x44, // 32 - roll
  0xC1, 0xC0, 0xC2, 0xAC, 0xAD, 0xAE, 0xC3, 0xC0, 0xC4, // 33 - roll lit
  0x41, 0x40, 0x42, 0x7D, 0x7E, 0x7F, 0x43, 0x40, 0x44, // 34 - bank
  0xC1, 0xC0, 0xC2, 0xFD, 0xFE, 0xFF, 0xC3, 0xC0, 0xC4  // 35 - bank lit
};

// Zero based indices into the table above
#define TILE_ROLL      31
#define TILE_ROLL_LIT  32
#define TILE_BANK      33
#define TILE_BANK_LIT  34
...
void drawDie(unsigned char x, unsigned char y, unsigned char s, bool isSelected, bool isHighlighted) {
  ...
  if (s>=1 && s<=6) {
    ...
    if (isSelected)
      idx += 6;          // kept
    if (isHighlighted)
      idx += 19;         // alternate color block
  } else if (s==18) {    // DIE_ROLL
    idx = isHighlighted ? TILE_ROLL_LIT : TILE_ROLL;
  } else if (s==17) {    // DIE_BANK
    idx = isHighlighted ? TILE_BANK_LIT : TILE_BANK;
  } else {
    return;
  }

  source = diceChars + idx*9;
...
// Verticals are the same 124 the box sides use; the tees weld it to the rules.
...
// The frame sits one row above the die and spans the three it occupies.
#define DICE_CURSOR_Y (DICE_Y - 1)

void drawDiceCursorInternal(unsigned char x, unsigned char offset) {
  static unsigned char i;
  static unsigned char* pos;

  pos = xypos(x-1,DICE_CURSOR_Y);
