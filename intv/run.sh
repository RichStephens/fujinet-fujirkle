#!/bin/sh
# run.sh -- build (if needed) and launch Fujirkle in the FujiNet-patched
...
cd "$(dirname "$0")"
# Exported, not just assigned: without the export SDL never sees it and
# auto-detects its audio backend instead -- which picks PipeWire on
# current systems and segfaults this jzIntv build on startup.
...
# last built.
if [ ! -f fujirkle.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer fujirkle.rom)" ]; then
    echo "Building fujirkle.rom..."
    make
fi
